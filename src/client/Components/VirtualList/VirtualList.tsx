...
export const VirtualList = <T extends object>(options: VirtualListOptions<T>) => (ListItemComponent: React.ComponentType<T>) => {
	const VList = class VirtualListInner extends React.PureComponent<Properties, VirtualListState> {
		...
		updateStateIfRequired(): void {
			...
			if (this._list_container_element_ref) {
				...
				// tslint:disable-next-line:max-line-length
				new_state.last_item_index = new_state.first_item_index + new_state.max_visible_rows_in_viewport * (new_state.items_per_row || 0);
				new_state.container_min_height = Math.ceil(
					max_item_heigth_of_current_row * options.items.length / (new_state.items_per_row || 0)
				);
				new_state.first_item_distance_to_document_top = getDistanceToDocumentTop(this._list_container_element_ref);
			...
			if (this.state.item_height) {
				...
				if (last_item_index !== this.state.last_item_index) {
					const new_state = {
						...this.state,
						last_item_index,
						container_padding_top: (last_row - this.state.max_visible_rows_in_viewport) * this.state.item_height,
						first_item_index: last_item_index - this.state.max_visible_rows_in_viewport * (this.state.items_per_row || 0)
					};
					this.setState(new_state);
				}
			}
